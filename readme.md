...
Logging will also stop when the device memory is full (approx 5.8 MB)

### Output Files

Data files are saved to the `/data/` directory on the ESP32's flash filesystem. Each run creates a new file with an auto-incrementing number: `droplogger_data_1.bin` etc.
...
| `a_magnitude(ms^-2)` | m/s² | Total Magnitude of Acceleration |
...
**Please Note: Logger must be wifi 30 cm of the laptop/PC to connect to wifi**
...
```bash
python unpack_droplogger_binary.py folder/with/bin_files
```

This produces converts all bin files in folder/with/bin_files to csv (back into the same folder).
...
| `drop_logger.py` | ESP32 | Core high-speed sensor logging to CSV |
